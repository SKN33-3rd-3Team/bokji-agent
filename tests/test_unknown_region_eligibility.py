...
@pytest.mark.parametrize("subject,minimum,maximum,expected,checked", [
    ("self", 19, None, "미확인", True),
    ("self", None, 18, "미충족", True),
    ("child", 19, None, "미확인", False),
])
def test_year_age_and_nonself_region_guards_survive_final_composition(
        subject, minimum, maximum, expected, checked):
    state, _ = state_and_store()
    original = state["subsidy_chunks"][0]
    item = replace(original, chunk=replace(original.chunk, metadata={
        **original.chunk.metadata, "age_basis": "year", "age_start": minimum,
        "age_end": maximum,
    }))
    state.update(as_of=date(2026, 9, 9), subsidy_chunks=[item], slots={
        "birth_date": "2007-12-31", "age": 18, "age_year_based": 19,
        "age_subject": subject,
    })
    store = FakeStore({"policy-a": [item]})
    state.update(determine_eligibility(state, store))
    verdict = state["eligibility_verdicts"][0]
    assert verdict["verdict"] == expected
    assert ("연령" in verdict["checked"]) == checked
    assert "지역" in verdict["unchecked"]
    ...
    assert final["answer_status"] == "partial" and final["final_citations"]
    assert NOTICE in final["final_answer"]
    assert "모든 자격 조건 충족" not in final["final_answer"]
    if expected == "미충족":
        assert "연령 조건 미충족" in final["final_answer"]
    if not checked:
        assert "대상자의 나이가 확인되지 않아" in final["final_answer"]
        assert "연령 조건 미충족" not in final["final_answer"]
    assert item.chunk.metadata["region_scope"] == "unknown"
    assert item.chunk.metadata["region_names"] == []


@pytest.mark.parametrize("subject,basis,expected,age_checked", [
    ("self", "year", "미확인", True),
    ("self", "international_age", "미충족", True),
    ("child", "year", "미확인", False),
    ("unknown", "international_age", "미확인", False),
])
def test_unknown_region_preserves_corrected_age_basis_and_subject_guard(subject, basis, expected, age_checked):
    state, _ = state_and_store()
    item = state["subsidy_chunks"][0]
    item = replace(item, chunk=replace(item.chunk, metadata={**item.chunk.metadata,
                   "age_start": 19, "age_end": None, "age_basis": basis}))
    state.update(as_of=date(2026, 9, 9), subsidy_chunks=[item], slots={
        "birth_date": "2007-12-31", "age": 18, "age_year_based": 19, "age_subject": subject})
    verdict = determine_eligibility(state, FakeStore({"policy-a": [item]}))["eligibility_verdicts"][0]
    assert verdict["verdict"] == expected
    assert ("연령" in verdict["checked"]) == age_checked
    assert "지역" in verdict["unchecked"] and NOTICE in verdict["reasons"]
    if not age_checked:
        assert "연령" in verdict["unchecked"]
        assert any("대상자의 나이가 확인되지 않아" in reason for reason in verdict["reasons"])

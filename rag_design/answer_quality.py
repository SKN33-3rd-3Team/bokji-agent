...
판정에서 제외할 것: "OO 조건 추가 확인 필요", "장애 여부·취업 상태는
확인되지 않았습니다", "중복수급 여부는 미확인" 처럼 챗봇이 **무엇을
확인하지 못했는지 스스로 밝히는 문장**은 정책에 대한 사실 주장이
아니라 투명성 안내문입니다 - [근거 문서]에 그 문장 자체가 없다는 이유로
"근거 없음"에 넣지 마세요. 오직 챗봇이 **확인됐다고 단정하거나 구체적인
값(금액·법령명·조건 등)을 실제로 제시한** 문장만 [근거 문서]와 대조하세요.

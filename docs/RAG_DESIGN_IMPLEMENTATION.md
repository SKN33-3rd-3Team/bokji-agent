...
- 지역 필터는 `region_names`의 exact intersection을 사용하고 `전국` 문서는 모든 지역 필터에 일치한다. 유효한 `unknown` + `region_names == []` 문서는 지역 미확정 후보로 통과시키며 전국으로 변환하거나 지역 자격 충족으로 해석하지 않는다. 지역 필터가 있을 때 누락·형식 오류 metadata와 명시적 지역 불일치는 계속 제외하고 날짜·나이 등 다른 필터도 유지한다. N9는 UNKNOWN 정책의 지역만 미확인으로 추가하고 확인된 다른 조건을 유지한다. 전체 판정은 기존 충족이면 미확인으로 낮추되 독립적으로 확인된 미충족은 보존한다. N13은 이 답변의 결정적 템플릿을 유지하고 N14는 최종 답변에 "지역 조건 추가 확인 필요"를 보장한다. 이는 실제 지역 자격 확인이 아니며 다른 불확실성과 근거 부족 보류도 유지한다. 지역 코드는 원천별 선택적 보조 metadata로 보존할 수 있지만 공통 필터에는 사용하지 않는다.
...
- 현행 시도 집합은 [행정안전부 2026-07-01 행정구역코드 변경](https://www.mois.go.kr/frt/bbs/type001/commonSelectBoardArticle.do?bbsId=BBSMSTR_000000000052&nttId=127039)을 기준으로 `전남광주통합특별시`를 포함하고 폐지된 `광주광역시`·`전라남도`를 정규 이름으로 받지 않는다. 구 완전수식 시군구명은 공식 변경표로 하위 지역을 확정할 때만 새 이름으로 변환한다. 구 최상위명 단독의 입력 정규화는 새 특별시 전체로 임의 확대하지 않는다. 정부24 수집의 미판정은 아래 national 기본값을 따른다.
- 정부24 수집기의 소관기관명 분류는 별도 제품 기본값을 사용한다. 누락·공백·미등록 기관·모호하거나 해석하지 못한 지역명 등 기존 UNKNOWN 반환 경로는 새 변환 시 `national`, `["전국"]`으로 기록하고 지역 보조 코드는 비운다. 이는 실제 전국 지원자격 확인이 아니다. 새 national 문서는 N9의 지역 UNKNOWN guard와 그에 따른 "지역 조건 추가 확인 필요" 문구를 유발하지 않는다. 기존/외부 UNKNOWN metadata와 검색·N9 보호는 유지하며 기존 데이터·색인을 소급 변경하지 않는다.

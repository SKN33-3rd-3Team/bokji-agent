...
> **서비스 계약 개정 (PR62, 2026-09-18)**: 아래는 1차 RAG 데이터 계약·설계 이력이다. 현재 서비스의 승인 결정·구현·검증 범위는 [백엔드 계약 추적표](../backend/README.md#원본-문서와-남은-계약-차이)를 따른다. 아래 공통 지역 필터의 unknown 후보 허용은 일반 상담 경로다. [API-14 자동 추천](AUTO_RECOMMENDATION_API.md)은 사용자 지역 누락 시 `region_scope="national"`과 `region_names=["전국"]`이 확인된 문서만 후보 제한 전에 선별한다. 지역 입력이 있으면 기존 지역 필터를 사용하되 최종 추천은 자격 충족만 남기고 전역 보류면 전부 제외한다. 원천 메타데이터를 전국·충족으로 승격하지 않는다.

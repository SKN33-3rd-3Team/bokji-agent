...
> **개정 안내 (PR62, 2026-09-18)**: 아래 계획·테스트 수·미해결 목록은 당시 구현의 역사 기록이다. 현재 계약과 담당은 [백엔드 계약 추적표](../backend/README.md#원본-문서와-남은-계약-차이)를 따른다. N10/N10a의 구조화 계산 입력·정수 경계·부분 응답은 [공용 D5 명세](AUTO_RECOMMENDATION_API.md#api-11-검증부분-응답재개), N11의 상호배타 metadata와 원문 다른 정책명에 따른 자동 판정은 `257cd45` 이후 **조건부**다. 아래 과거의 `불가` 및 “조건부/산술 미구현” 설명을 현행 계약으로 적용하지 않는다.
>
> 일반 상담은 지역 미확정 후보·보류 카드를 유지한다. API-14는 지역 누락 시 전국 대상만 검색하고 자격 충족만 남기며, 전역 abstained면 모두 제외한다. LLM 노드 한도·최종 제공자 오류의 자동 모드 처리도 [현재 실행 계약](RUNPOD_SETUP_DRAFT.md#노드-총-실행-한도)을 따른다. 이 개정 안내가 아래의 과거 검증 결과를 새 코드 검증으로 바꾸지는 않는다.

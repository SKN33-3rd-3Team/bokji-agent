...
여기서 나오는 지표는 전부 **정책 ID 단위**(맞는 정책을 골랐는가)다. 생성된
답변 문장 자체가 사실과 맞는지(Faithfulness)·질문에 실제로 답했는지(Answer
relevancy)까지 보려면 이 파이프라인을 그대로 감싸는
`scripts/run_model_evaluation.py`(`docs/MODEL_EVALUATION.md`)를 쓴다 — 출력
파일명에 모델명·실행 날짜가 들어간다.

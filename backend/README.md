...
| --- | --- |
| 마케팅 동의 수정: 요구사항 `S-02!E10`, API `API-01!A59` | 가입·조회는 가능하지만 API-05 요청 정의와 `UpdateProfileRequest`/`update_profile()`에는 수정 필드가 없다. 수정 요구사항은 유효한 계약 공백으로 남는다 |
| 생년월일: 요구사항 `S-02!D7/E7`, API `API-01!D14` | 인증은 UTC 오늘의 정확히 120년 전 날짜를 하한으로, 그래프는 만 나이 120세를 상한으로 사용한다. 원본도 날짜 하한과 그래프 동일 경계를 함께 요구하므로 단일 기준 결정이 필요하다 |
| API-10 프리필 검증: `API-10!D16:D21` | 성별·장애·보훈·소득·날짜 `known_*`가 자유 문자열이며 잘못된 값은 코어에서 조용히 무시될 수 있다. 명시된 허용값의 서버 검증 공백이다 |
...
| 중복수급 판정: 요구사항 `S-07!E8` | 기존 N11은 제한 원문에 이번 응답의 다른 정책명이 명시되면 `불가`를 반환할 수 있다. “불가로 자동 격상하지 않음” 설명과 충돌하며 이번 API 작업에서 새로 생긴 동작은 아니다 |
| 회원 등급: 요구사항 `S-09!E6/H6`, API-04 | 고정 “일반 회원” 응답 요구와 추후 필드 추가 설명이 충돌하며 현재 API-04에는 등급 필드가 없다 |

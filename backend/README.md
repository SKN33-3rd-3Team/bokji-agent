...
- 비밀번호 변경은 현재 세션을 포함해 기존 로그인 세션을 유지한다. 탈퇴 시 로그인 토큰 폐기와 상담 정리 범위는 [아래 설명](#상담-실패와-회원-탈퇴-처리)을 따른다. API-04/05/08의 프로필 응답은 `Cache-Control: no-store`다.
...
- API-10은 그래프 실행부터 응답 구성·검증·소유권 등록·캐싱까지 실패 처리를 적용한다. 실패하면 실제 실행한 그래프의 해당 `session_id` 체크포인트와 부분 등록된 소유권·응답 캐시를 각각 정리한다. 다른 상담과 정상 응답(`answered`/`needs_input`)의 상태는 보존한다.
...
- 내부적으로 [service.ask()](../src/rag_chatbot/service.py)에 선택적 `_on_graph_ready=None` 콜백을 추가했다. 백엔드가 실제 실행한 그래프를 전달받아 정리하며, 정리하려고 그래프를 다시 초기화하지 않는다. 기존 호출부는 콜백을 생략할 수 있고, 그래프의 검색·정책 판정 로직과 HTTP 요청·응답 형식은 유지한다.

### 회원별 실행 순서와 탈퇴

- API-07(탈퇴)과 API-10~13(상담 시작·재개·정책 문의·삭제)은 같은 회원 ID의 잠금을 공유해 순차 처리한다. 같은 회원의 서로 다른 상담도 순차 처리되며, 다른 회원은 이 잠금 때문에 대기하지 않는다. 잠금은 프로세스 내부에 있으므로 위의 단일 프로세스 운영 전제가 적용된다.

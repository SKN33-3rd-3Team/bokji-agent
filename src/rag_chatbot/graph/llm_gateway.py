...
# 검색 질의용 인적사항 제거
# ---------------------------------------------------------------------------
# 검색 질의(N4)는 "이 사람이 누구인가"가 아니라 "무엇을 찾는가"를 표현해야
# 한다. 지역·나이·성별·소득구간·장애유무는 이미 구조화 슬롯으로 뽑혀
# ``VectorSearchFilter``(지역·나이)와 ``filter_candidates``(성별·소득·장애)가
# 처리하므로, 같은 정보를 임베딩 벡터에도 넣으면 신호가 아니라 **중복**이다.
# 임베딩 차원의 상당 부분을 "서울 남성 중위소득 50%"를 표현하는 데 쓰면
# 정작 주제어("근로장려금")가 차지할 자리가 줄어든다.
#
# 2026-09-15 Dev 100문항 실측: 질의에서 인적사항을 뺐을 때 정답 정책의
# Top-5 적중이 62/95 -> 85/95로 올랐다.
#
# 직업·종사 분야는 **일부러 남긴다**. 성별·소득구간·장애유무는 닫힌 어휘의
# 상용구라 주제 정보가 없지만("여성입니다", "비장애"), "어업에 종사" 같은
# 직업 표현은 열린 어휘이고 정책 주제("어선 장비 지원")와 직접 겹친다.
#
# 문장 위치에 의존하지 않는다는 점이 중요하다. "마지막 문장만 남긴다" 같은
# 규칙은 질문이 "인적사항 문장 + 요구 문장" 순서로 쓰였다는 가정에 기대는데,
# 실제 발화는 그 순서를 지키지 않는다(실측: 마지막 문장이 "관련 지원이
# 있나요?"인 질문에서 주제어가 앞 문장에 있어 상위권 -> 권외로 회귀).
#
# 성별·소득구간 모두, 자기소개 나열("여성입니다", "여성, 중위소득...")에서만
# 쓰이는 뒤따르는 표시(자기소개 어미 또는 나열용 쉼표)가 바로 붙어 있을
# 때만 지운다. 이 표시가 없으면(예: "여성 1인 가구 주거지원", "중위소득
# 60% 이하 가구가 받을 수 있는 지원") 자기소개가 아니라 질문 자체의 주제어일
# 가능성이 높으므로 장애 패턴과 같은 원칙으로 남긴다.
_SELF_REPORT_SUFFIX = r"(?:입니다|이에요|예요|이며|이고|임)"
_GENDER_PHRASE_PATTERN = re.compile(
    rf"(?:여성|여자|남성|남자)(?=\s*(?:,|{_SELF_REPORT_SUFFIX}))"
    rf"(?:\s*{_SELF_REPORT_SUFFIX})?"
)
# "소득은 중위소득 80%이고" 전체를 한 덩어리로 지운다. 숫자만 지우면
# "소득은 이고" 같은 잔해가 남아 오히려 더 시끄럽다.
_INCOME_PHRASE_PATTERN = re.compile(
    r"(?:(?:가구\s*)?소득(?:은|이|도)?\s*)?"
    r"(?:기준\s*)?중위\s*소득\s*(?:대비\s*)?(?:의\s*)?\d{1,3}\s*(?:%|퍼센트)"
    rf"(?:\s*(?:이하|이상|초과|미만))?(?=\s*(?:,|{_SELF_REPORT_SUFFIX}))"
    rf"(?:\s*{_SELF_REPORT_SUFFIX})?"
)
# 상태 표현만 지운다. 맨 "장애인"은 남긴다 - "장애인 활동지원 알려주세요"의
# "장애인"은 인적사항이 아니라 주제어다(그리고 _INTEREST_KEYWORDS에도 있다).
_DISABILITY_PHRASE_PATTERN = re.compile(
    r"장애(?:는|가)?\s*없(?:음|고|으며|습니다|어요|는)"
    r"|(?:비장애(?:인)?"
    r"|장애(?:는|가)?\s*없"
    r"|등록\s*장애(?:인)?|장애\s*등록|장애인\s*등록"
    r"|장애인"
    r"|(?:중증|경증|청각|언어|시각|지체)\s*장애(?:가|인)?"
    r"|장애가\s*있)"
    rf"(?=\s*(?:{_SELF_REPORT_SUFFIX}|있(?:음|고|으며|습니다|어요)|했(?:습니다|어요)|함|임))"
)
# 생년월일은 redact_sensitive_text가 이미 지운다. 그 자리에 홀로 남는 "생"과
# 지역 표현에 붙는 "거주/에 사는"은 우리 자신의 제거가 만든 잔해라 같이 치운다.
# "사는"은 반드시 "에"를 동반할 때만 지운다. 맨 "사는"까지 지우면
# "혼자 사는데 월세가 부담돼요"의 "사는데"가 깨진다.
_PROFILE_RESIDUE_PATTERN = re.compile(
    r"(?<![가-힣0-9])생(?![가-힣])"
    r"|에\s+(?:사는|살고\s*있(?:는|습니다|어요))"
    r"|(?<![가-힣])거주(?:(?![가-힣])|(?=\s*(?:중|하고\s*있|합니다|이에요|예요|임)))"
)
# (정규식, 지울 최대 개수). 0은 "전부".
#
# 소득·장애·성별을 **한 번만** 지우는 이유: 자기소개는 한 번 하지만, 같은
# 표현이 두 번째로 나오면 그건 자기소개가 아니라 질문 내용이다. 전부 지우면
# "중위소득 80%입니다. 중위소득 60% 이하 대상 기준을 알려주세요"에서 정작
# 물어본 기준까지 사라진다(실측으로 확인). 지역은 하드 필터라 몇 번 나오든
# 질의에서 빼는 편이 낫고, 잔해 패턴은 우리가 만든 공백이라 전부 지운다.
#
# 나이("70세")는 아예 지우지 않는다. "70세 이상 해양사고관련자의 지원 조건"
# 처럼 **제도의 자격 요건**을 묻는 표현이 압도적으로 많고, 자기 나이는
# 대부분 생년월일로 들어와 ``redact_sensitive_text``가 이미 지운다. 남겨서
# 잃는 것(토큰 하나)보다 지워서 잃는 것(질문의 핵심 조건)이 크다.
_PROFILE_PHRASE_PATTERNS: tuple[tuple[re.Pattern[str], int], ...] = (
    (_REGION_PATTERN, 0),
    (_INCOME_PHRASE_PATTERN, 1),
    (_DISABILITY_PHRASE_PATTERN, 1),
    (_GENDER_PHRASE_PATTERN, 1),
    (_PROFILE_RESIDUE_PATTERN, 0),
)
# 제거 후 남은 구두점·공백 정리용.
_PUNCT_RUN_PATTERN = re.compile(r"[,\s]*,[,\s]*")
_SPACE_RUN_PATTERN = re.compile(r"\s{2,}")


def strip_profile_phrases(text: str) -> str:
    """검색 질의에서 **이미 필터가 처리하는** 인적사항 표현을 걷어낸다.

    지우는 대상은 위 ``_PROFILE_PHRASE_PATTERNS``에 한정한다. 남는 것이
    없으면 빈 문자열을 돌려주므로, 호출하는 쪽에서 원문으로 되돌릴지
    정해야 한다(인적사항만으로 이뤄진 발화가 실제로 있다).

    알려진 한계: 자기소개가 질문보다 **뒤**에 오면서 같은 표현을 쓰는 경우
    ("중위소득 60% 이하가 대상인가요? 저는 80%입니다")에는 첫 번째 것,
    즉 질문 쪽을 지운다. 이때도 질의가 비지는 않으므로 검색이 실패하지는
    않고 예전처럼 희석된 상태로 돌아간다. fail-open 방향의 오차다.
    """

    stripped = text
    for pattern, limit in _PROFILE_PHRASE_PATTERNS:
        stripped = pattern.sub(" ", stripped, count=limit)
    stripped = _PUNCT_RUN_PATTERN.sub(", ", stripped)
    stripped = _SPACE_RUN_PATTERN.sub(" ", stripped)

...
### 개발 평가 통합 결과 (Dev 150 + Policy-eval 100 + 추가 Dev 100 = 총 350문항)
...
참고용 이전 통합치(v0, Dev 150+Policy-eval 100=250건만 - 추가 Dev는
...
추가 Dev 세 세트 모두에서 40~70% 폭으로 일관되게 나타나(4절), 특정
개발 세트 모두에서 개선을 관찰했다. 독립 Holdout 검증은 남아 있다.
...
0.574(v1)→0.738, 추가 Dev(v3 대비): 0.763→0.778. 추가 Dev에서도 개선 방향은 같지만, 반복 평가한 세트이므로 독립적인 일반화 검증으로 해석할 수 없다. 개선 폭도 Dev·Policy-eval과 다르다. Dev 150의 v4 Faithfulness는 113/150건
...
당연한 결과). 반면 추가 Dev(v3→v4)에서는 Precision 0.75→**1.000**
(오탐 0건), Recall 0.30→0.45로 **정밀도와 재현율이 동시에** 개선됐다
- 세 세트 중 유일하게 둘 다 좋아진 경우다. 두 세트가 다른 방향을
보이는 건 표본 크기(Dev 8건 vs 추가 Dev 20건)와 보류 문항 성격
...
**전제**: 기존 `quasi_holdout_questions.jsonl`은 v3/v4 등 여러 버전에서
반복 평가했으므로 개발용 추가 검증 세트로 분류한다. 기존 파일명은 결과 추적을
위해 유지한다. 이 보고서의 350문항 통합치는 개발 평가이며 Gate 6 최종 Holdout
검증을 대체하지 않는다. 별도로 동결한 미사용 Holdout의 최종 평가는 남아 있다.

1. **과적합이 감소했는가?** → 현재 수치만으로 단정할 수 없다. 여러 개발
   세트에서 Faithfulness 개선 방향을 관찰했지만 변화 폭과 판정 표본이 다르고,
   반복 확인한 세트이므로 독립 검증으로 볼 수 없다.
2. **추가 Dev에서 성능이 유지되는가?** → 유지되는 정도가
...
3. **Dev와 추가 Dev의 metric gap이 줄었는가?** → Faithfulness 기준으로는
   격차가 거의 없다(Dev 0.752 vs 추가 Dev 0.778, 차이 0.026). Recall
   기준으로는 격차가 있다(Dev 0.810 vs 추가 Dev 0.750, 차이 0.06) -
   이 격차가 원래부터 있었는지(v3에서 처음 측정하므로 "전"이 없다)
   비교할 기준점이 없어 "줄었다/늘었다"를 말할 수 없다. 최종 일반화 검증에는 별도로 동결한 미사용 Holdout이 필요하다.
4. **특정 데이터셋에만 성능이 좋은가?** → 현재 개발 세트만으로 단정할 수 없다. 과적합 감사(코드
   레벨)에서 근거를 못 찾았고, 수치 레벨에서도 Faithfulness 개선이
   세 세트에서 관찰됐지만 독립 Holdout 검증은 남아 있다.
5. **어떤 질문 유형에서 여전히 실패하는가?** → 주제어가 거의 없는
   되묻기형 질문(관련성 게이트, Dev에서 확인), 법령명·조건 항목을
   세부적으로 틀리는 경우(N9 조건 대조 정교화 필요, 이번 patch로
   안 잡힘), 추가 Dev에서 Recall/MRR이 낮게 나오는 원인(미조사).
...
- ~~Citation Precision이 v1/v2 대비 v3/v4에서 후퇴함~~ → **v5에서
  ...
  **Abstention Precision/Recall이 350문항 통합 기준 0.812/0.464→
  ...
  엇갈려(Dev는 개선, Policy-eval·추가 Dev는 후퇴) 8~20건짜리 작은
...
- 추가 Dev 세트의 Recall(0.750)·MRR(0.548)이 세 세트 중 가장
...
6. ~~`_CONFIDENT_DISTANCE_THRESHOLD`를 낮춰 LLM 게이트를 더 자주 거치게
   ...
   더 큰 보류 문항 표본(현재 Dev 8건, 추가 Dev 20건)으로 재검증할 것.
7. 추가 Dev의 낮은 Recall/MRR 원인 조사 - 실패 사례를

...
def run_pipeline(
    *,
    ...
    ``extra_interests``(사이드바에서 고른 지원조건·관심 분야)와
    ``known_region``(회원 프로필의 거주 지역)은 첫 질문에만 전달한다.
    재개 중에는 체크포인터의 슬롯이 이미 확정돼 있어서 ``answer_followup``이
    이 인자들을 받지 않는다.
    """
    ...
    return ask(
        user_input, session_id, top_k=top_k, extra_interests=extra_interests,

...
from scripts.collect_60_results import DEFAULT_BASE, load_selection

DEFAULT_DEST = Path("output/llm_comparison_60_runpod_20260916")
...
def questions(base):
    ...
    return {"workbooks": [{"filename": "60문장_질문과_답변기준.xlsx", "title": "60문장 질문과 답변 기준", "sheets": [
        sheet("읽는 방법", ["항목", "설명"], [
            ["구성", f"{len(scenarios)}가지 상황 × 같은 뜻의 표현 3개 = {len(cases)}문장. 말투별 문장 수: " + ", ".join(f"{k} {v}개" for k, v in sorted(styles.items()))],
        ...
        sheet("문장 60개", ["문장 ID", "상황 ID", "질문 종류", "말투", "질문", "정책"],
...
def results(base):
    ...
    if summary.get("selection_sha256") != selection_hash:
        raise ValueError("Summary selection mismatch")
    rows = read(base / "reviewed_results.json")
    expected = {(m, c["id"]) for m in LABELS for c in cases}
    if len(rows) != len(expected) or {(r["model"], r["case"]["id"]) for r in rows} != expected:
        raise ValueError("All 180 reviewed outputs are required")
    by_model = summary["models"]
    for model in LABELS:
        if by_model[model]["n1_count"] != sum(c["n1_applicable"] for c in cases):
            raise ValueError("N1 count differs from selection")
    names = list(LABELS)
    metrics = [
        ("도움이 되고 근거에 맞는 최종 답변 / 60", "detail_success"),
        ("근거 없는 주장이 있는 최종 답변 / 60", "detail_unsupported"),
        ("세 말투 모두 성공한 상황 / 20", "all_three_success"),
        ("말투에 따라 성공 여부가 달라진 상황 / 20", "style_sensitive_situations"),
        ("세 말투 모두 실패한 상황 / 20", "all_three_failed"),
        ("조건 이해 평균 / 5점", "n1_mean"), ("조건 이해 점수에 포함한 문장 수", "n1_count"),
        ("질문에 없는 개인 조건을 추가 / 60", "n1_invented"),
    ...
    sheets = [
        sheet("결과 요약", ["항목", *LABELS.values()], overview, [65, 24, 24, 24],
              "AI 검토 참고 결과입니다. 60문장은 20상황의 말투 변형입니다. 실패에는 잘못된 답변·설명 누락·서비스 차단이 포함됩니다. 점수·건수는 상세 시트 수식으로 계산하며 긴 답변은 첫 행만 셉니다. 시간·호출 통계는 고정 실행 요약 값입니다."),
        ...
        sheet("말투별", ["말투", "모델", "문장 수", "성공", "근거 없는 주장"], style_rows, [26, 26, 15, 15, 22],
              "말투별 문장 수는 선택된 질문 기준입니다. 문장 길이·오타 등이 함께 다른 조건이므로 말투만의 순수 효과라고 단정할 수 없습니다."),
    ...
    return {"workbooks": [{"filename": "60문장_세모델_비교결과.xlsx", "title": "60문장 세 모델 비교 결과", "sheets": sheets}]}
...
if __name__ == "__main__":
    ...
    parser.add_argument("--destination", type=Path, default=DEFAULT_DEST, help="Helper JSON directory; no XLSX is rendered")
    args = parser.parse_args()
    value = questions(args.output) if args.questions else results(args.output)
    args.destination.mkdir(parents=True, exist_ok=True)
    write_new(args.destination / ("question_workbook.json" if args.questions else "result_workbook.json"), value)

#!/usr/bin/env bash
# ---------------------------------------------------------------------------
# [DEPRECATED - 2026-09-16] 회원 DB 를 RunPod 에서 skn33.iptime.org 로
# 옮기면서 더 이상 쓰지 않는다. 그 서버는 이미 MariaDB 가 떠 있으므로 계정
# 생성만 하면 되고(scripts/setup_sing_up_db.sql), 이 스크립트가 하던
# "빈 Ubuntu Pod 에 MariaDB 를 설치해 올리는" 작업 자체가 필요 없다.
# RunPod Pod 를 다시 쓰게 되면 그때 참고용으로 남겨둔다.
# ---------------------------------------------------------------------------
#

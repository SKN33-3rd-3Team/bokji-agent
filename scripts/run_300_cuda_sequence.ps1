...
if ($taskRecord.backend -ne 'cuda_v13' -or $taskListener.OwningProcess -ne $taskRecord.pid -or
    $taskProcess.ExecutablePath -ne $taskRecord.executable) { throw 'Owned CUDA server identity mismatch' }
$env:OLLAMA_BASE_URL = 'http://127.0.0.1:11435'
$env:PYTHONIOENCODING = 'utf-8'
$taskPython = Join-Path $taskRoot '.venv/Scripts/python.exe'
$taskWrapper = Join-Path $PSScriptRoot 'run_300_guarded.py'
foreach ($taskEntry in @(@{model='ax'; layers=27}, @{model='bllossom'; layers=27}, @{model='qwen'; layers=30})) {
    & $taskPython $taskWrapper --model $taskEntry.model --num-gpu $taskEntry.layers `
        --output $taskOutput --server-record $taskRecordPath
    if ($LASTEXITCODE -ne 0) { throw "Stopped at $($taskEntry.model); records preserved for investigation" }
}
